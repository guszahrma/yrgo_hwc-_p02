...
Esp32s3::Esp32s3(int baudRate) noexcept
    ...
    , myInternalLen{}
{
    //! @note Why do pass a baud rate if it isn't used? I would remove it if I were you.
    (void)baudRate;

    //! @note Avoid magic numbers; consider using your constant InternalBufLen, which is 256.
    //!       Also please skip =, {} will do.
...
{
    //! @note Place parenthese around the conditions, i.e. 
    //!       if ((nullptr == buffer) ||(0U == maxSize)).
    if (nullptr == buffer || 0U == maxSize) { return false; }

    std::uint8_t byte{};
    
    //! @note I would use Yoda here as well.
